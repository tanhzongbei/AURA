...
class Test(unittest.TestCase):
    ...
    def test1(self):
        ...
        username2, token2, nickname2, userid2 = self.regist()


        data = {'token' : token, 'followeeid' : userid2}
        data = ujson.dumps(data)
        ...
        data = {'token' : token2, 'followeeid' : userid1}
        data = ujson.dumps(data)
        res = self.api('follow', data)
        assert res['result_code'] == 10000

        data = {'token' : token, 'userid' : userid2}
        data = ujson.dumps(data)
        res = self.api('queryAllFollowerByUid', data)
        assert res['result_code'] == 10000

        data = {'token' : token, 'userid' : userid2}
        data = ujson.dumps(data)
        res = self.api('queryAllFolloweeByUid', data)
        assert res['result_code'] == 10000

        data = {'token' : token, 'userid' : userid2}
        data = ujson.dumps(data)
        res = self.api('queryUserInfo', data)
        assert res['result_code'] == 10000
        print res
